...
function scoreUrgency(candidate: ImprovementCandidate): number {
  const trigger = candidate.triggerCount;
...
export async function generatePriorityQueue(): Promise<PrioritizedFix[]> {
  ...
  for (const line of lines) {
    ...
    for (const reflection of entry.reflection ?? []) {
      ...
      if (!issues[key]) {
        issues[key] = {
          sourceModule: reflection.sourceModule ?? 'unknown',
          issue: reflection.issue ?? 'unspecified',
          proposedFix: reflection.proposedFix ?? '',
          triggerCount: 1,
          confidenceTrend: reflection.confidenceTrend ?? []
        };
      } else {
        issues[key].triggerCount += 1;
